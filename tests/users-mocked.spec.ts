...
async function setupUsersMocks(page: Page) {
  ...
  const usersPage0 = {
    users: [
      { id: '10', name: 'Kai Chen', email: 'd@jwt.com', roles: [{ role: Role.Diner }] },
      { id: '11', name: 'Buddy', email: 'b@jwt.com', roles: [{ role: Role.Admin }] },
    ],
    more: true,
  };
  const usersPage1 = {
    users: [{ id: '12', name: 'Lara', email: 'l@jwt.com', roles: [{ role: Role.Diner }] }],
    more: false,
  };
  ...
  await page.route('**/api/auth', async (route) => {
    ...
    await route.fulfill({ status: 200, json: { user: adminUser, token: 'admin-token' } });
  });
  ...
  await page.route('**/api/user**', async (route) => {
    ...
    if (url.includes('name=*Kai*')) {
      await route.fulfill({ status: 200, json: { users: [usersPage0.users[0]], more: false } });
      return;
    }
