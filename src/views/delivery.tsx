...
export default function Delivery() {
  ...
  const [jwtPayload, setJwtPayload] = React.useState<JWTPayload>({
    message: "invalid",
    ...
    <View title="Here is your JWT Pizza!">
      <div className="my-4 flex max-w-xl justify-center items-center flex-col  py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="my-4 text-lg text-orange-200 text-start grid grid-cols-5 gap-2">
          <div className="font-semibold text-orange-300">order ID:</div>{" "}
          <div className="col-span-4">{order.id}</div>
          <div className="font-semibold text-orange-300">pie count:</div>{" "}
          <div className="col-span-4">{order.items?.length}</div>
          <div className="font-semibold text-orange-300">total:</div>{" "}
          <div className="col-span-4">
            {order.items
              ?.reduce((a: number, c: any) => a + c.price, 0)
              .toLocaleString()}{" "}
            ₿
          </div>
        </div>
        ...
        <div
          className={`font-thin break-all font-mono text-xs bg-slate-100 p-2 ${jwtPayload.message === "valid" ? "text-green-700" : "text-red-700"}`}
        >
          {jwt}
        </div>
      </div>
